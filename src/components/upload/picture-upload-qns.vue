  ...
  <div>
    <div class="picture-upload-qn-wrapper">
      <ul class="photos-wrapper">
        <li
          v-for="(item,index) in picListimg"
          class="photos"
        ...
        <li
          v-if="picListimg.length<maxNum"
          class="upload-btn"
          :style="{'width':`${width}px`, 'height':`${width}px`, 'borderRadius':`${borderRadius}px`}"
        >
          <label :for="fileId" class="btn" @change="selectFile">
            <input v-show="false" :id="fileId" ref="file" type="file" multiple="multiple">
          </label>
        </li>
      </ul>
...
import { getImageToken } from "@/api/hs/member"
export default {
  components: {
    PictureCheck
  },
  ...
  methods: {
    ...
    async selectFile() {
      ...
      const cl = this.picListimg.length
      let index = cl
      // 将本次选择的文件组合成数组集合
      const selectFiles = []
    ...
    fileUpload(file, index) {
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            this.picListimg[index].link = `${this.domain}/${res.hash}`
            this.picListimg[index].load = true
            this.currentNum += 1
      ...
      xhr.upload.onprogress = (evt) => {
        if (evt.lengthComputable) {
          this.picListimg[index].progress = parseInt(Math.round(evt.loaded * 100 / evt.total))
        }
      }
      // 发送
      xhr.send(formData)
    ...
    // 删除图片
    handleDel(index) {
      this.picListimg.splice(index, 1)
      this.finishUploadAll()
      this.$emit('finishdel', index)
    },
    // 对外的接口,输出最新的图片地址集合
    finishUploadAll() {
      const uploadUrl = this.picListimg.map((item) => {
        return item
      })
      this.$emit('shabi', uploadUrl)
    },
    // 查看图片
    handleCheck(index) {
