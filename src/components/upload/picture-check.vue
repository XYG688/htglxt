<template>
  <div class="picture-check-wrapper" @click="handleClose">
    ...
    <div class="menu">
      <div v-if="currentIndex>0" class="icon left" @click.stop="handlePrev"><img src="@/assets/image/upload/leftArrow.png" alt=""></div>
      <div v-if="currentIndex<list.length-1" class="icon right" @click.stop="handleNext"><img src="@/assets/image/upload/rightArrow.png" alt=""></div>
    </div>
    <div class="page">
      <span v-for="(i,index) in list" :class="{active: currentIndex===index}" @click.stop="handleMove(index)" />
    </div>
  ...
  .picture-check-wrapper{
    ...
    .close{
      ...
      &:after{
        ...
        width: 20px;
        height: 20px;
        background: url("../../assets/image/upload/close.png") no-repeat center center / cover;
      }
    }
  }
</style>
