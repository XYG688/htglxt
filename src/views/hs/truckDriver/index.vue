  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="车牌号" prop="numberPlate">
        <el-input
          v-model="queryParams.numberPlate"
          placeholder="请输入车牌号"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
      ...
      <el-form-item label="是否到达东兴" prop="enterState">
        <el-select v-model="queryParams.enterState" placeholder="请选择审核状态" clearable size="small">
          <el-option
      ...
      <el-form-item label="始发地" prop="origin">
        <el-select v-model="queryParams.origin" placeholder="请选择始发地" size="small" clearable @clear="clearCompany">
          <el-option :value="queryParams.originId" :label="queryParams.origin"
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-delete"
          :disabled="multiple"
          v-hasPermi="['permit:driver:remove']"
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleAudit"
          v-hasPermi="['permit:driver:audit']"
        >一键审核
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleReject"
          v-hasPermi="['permit:driver:reject']"
        >一键驳回
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          :disabled="multiple"
          @click="handleCirculation"
          v-hasPermi="['permit:driver:circulation']"
        >一键流转归属
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['permit:driver:export']"
        >导出
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="driverList" @selection-change="selectionChangeHandle" class="masterTable">
      ...
      <!--       <el-table-column label="同乘人" align="center" prop="userPeerCnt">-->
      <!--        <template slot-scope="scope">-->
      <!--          <el-link  type=""  href="javascript:void(0);" class="master-info" :underline="false"-->
      <!--            @click.native.prevent="toInfoPage( scope.row)">{{scope.row.userPeerCnt}}</el-link>-->
      <!--        </template>-->
      <!--      </el-table-column>-->
      <el-table-column label="目的货场(公司)" align="center" prop="destCnt" width="180">
        <template slot-scope="scope">
          <el-link type="" href="javascript:void(0);" class="master-info" :underline="false"
                   @click.native.prevent="toInfodestCnt( scope.row)">{{ scope.row.destCnt }}
          </el-link>
        </template>
      </el-table-column>
      <el-table-column label="始发地" align="center" prop="origin" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column label="24小时核酸证明" align="center" prop="hsUrl">
        <template slot-scope="scope">
      ...
      <el-table-column label="是否到达东兴" align="center" prop="enterState">
        <template slot-scope="scope">
    ...
    <div class="other">
      <async-modal :dialogVisible="dialogVisible" :title="modalTitle" @closeDialog="closeDialog" :width="'1300px'"
                   :isShowScroll="true">
        <el-form :model="driverForm" ref="queryForm" :inline="true" label-width="220px">
          <el-row>
            <el-col :span="12">
              <el-form-item label="姓名" prop="name">
              ...
              <el-form-item label="始发地" prop="origin">
                <el-select v-model="driverForm.origin" placeholder="请选择始发地" style="width:320px" size="small" clearable
                           @clear="clearCompany" :disabled="editFlag === 0">
                  <el-option :value="driverForm.originId" :label="driverForm.origin"
                             style="height:200px;overflow: auto;background-color:#fff">
                    <el-tree
                      v-if="originTreeList.length > 0"
                      :data="originTreeList"
              ...
              <el-form-item label="预计进东兴时间" prop="enterDate">
                <el-date-picker v-model="driverForm['userReport'] && driverForm['userReport'].enterDate" type="date"
                                ...
                                class="formWidth" style="width:320px" :disabled="editFlag === 0"></el-date-picker>
              </el-form-item>
              <!-- <el-form-item label="货物种类" prop="goodsType">
               <async-selector v-model="driverForm['goodsType']"  :address="'/system/dict/data/type/goods_type'" placeholder="请选择始发地 " class="input-range1" :disabled="editFlag === 0"></async-selector>
              </el-form-item> -->
              <el-form-item label="是否当天返回" prop="returnSameDay">
                <div class="input-range">
                  <el-select v-model="driverForm['returnSameDay']" placeholder="请选择" class="input-range1"
                             :disabled="editFlag === 0">
                    <el-option
                  ...
                  <el-input v-model="driverForm.overnightDays" placeholder="" class="input-range2"
                            :disabled="editFlag === 0"></el-input>
                  天
                </div>
              </el-form-item>
              <el-form-item label="14天内是否发热,咳嗽登不适症状" prop="physicalAbnormality">
                <el-select v-model="driverForm['physicalAbnormality']" placeholder="请选择" class="formWidth"
                           style="width:320px" :disabled="editFlag === 0">
                  <el-option
                    v-for="item in options"
                    :key="item.id"
                    :label="item.name"
            ...
            <el-col :span="12">
              ...
              <el-form-item label="始发地详细地址" prop="originDetail">
                <el-input v-model="driverForm.originDetail" class="formWidth" :disabled="editFlag === 0"/>
              </el-form-item>
              <!-- <el-form-item label="目的货场(公司)" prop="destCnt">
              <el-input v-model="driverForm.destCnt" class="formWidth" disabled/>
             </el-form-item> -->
              <el-form-item label="是否需要代驾" prop="">
                <div class="input-range">
                  <el-select v-model="driverForm['chauffeuring']" placeholder="请选择" class="input-range1"
                             :disabled="editFlag === 0">
                    <el-option
                      v-for="item in options"
                      :key="item.id"
                  ...
                  <el-input v-model="driverForm.chauffeuringName" placeholder="代驾姓名" class="input-range3"
                            style="width:80px" :disabled="editFlag === 0"></el-input>
                  <el-input v-model="driverForm.chauffeuringPhone" placeholder="代驾手机号码" class="input-range2"
                            :disabled="editFlag === 0"></el-input>
                </div>
              </el-form-item>
              <el-form-item label="司机类型" prop="driverType">
                <el-select v-model="driverForm['driverType']" style="width:320px" placeholder="请选择始发地"
                           :disabled="editFlag === 0">
                  <el-option
                    v-for="dict in dict.type.driver_type"
                    :key="dict.value"
                    :label="dict.label"
                ...
                <!-- <async-selector v-model="driverForm['driverType']" :address="'/system/dict/data/type/driver_type'" placeholder="请选择始发地 " :disabled="editFlag === 0"></async-selector> -->
              </el-form-item>
              <el-form-item label="14天内是否到达过中高风险地区" prop="highRisk">
                <el-select v-model="driverForm['highRisk']" placeholder="请选择" class="input-range1" style="width:320px"
                           :disabled="editFlag === 0">
                  <el-option
                    v-for="item in options"
                    :key="item.id"
          ...
          <el-row>
            <el-col :span="10.6">
              <el-form-item label="网格证状态" prop="userState">
                <el-select v-model="driverForm['userReport'] && driverForm['userReport'].userState"
                           :disabled="editFlag === 0" placeholder="状态" clearable size="small" style="width:320px">
                  <el-option
                    v-for="dict in dict.type.permit_state"
                    :key="dict.value"
                    :label="dict.label"
        ...
        <div class="bothSides">
          <p>目的地</p>
          <!-- <p @click="addLine()" v-show="editFlag !== 0">+</p> -->
        </div>
        <el-table ref="myTable" border stripe :data="tableList" :header-cell-style="{'text-align':'center'}">
          <el-table-column label="序号" type="index"></el-table-column>
          <el-table-column label="目的地名称" prop="spName">
            <template slot-scope="{ row }">
              <template v-if="editFlag === 2">
                <el-input v-model="row.spName" class="edit-input"/>
              </template>
              <span v-else>{{ row.spName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="是否到达" prop="confirm">
            <template slot-scope="{ row }">
              <template v-if="editFlag === 2">
                <el-select v-model="row.confirm" placeholder="请选择" class="formWidth" style="width:277px">
                  <el-option
                    v-for="item in options"
                    :key="item.id"
                    :label="item.name"
              ...
              <span v-else>{{ row.confirm ? '是' : '否' }}</span>
            </template>
          </el-table-column>
          <el-table-column label="到达时间" prop="confirmDate">
            <template slot-scope="{ row }">
              <template v-if="editFlag === 2">
                <el-date-picker v-model="row.confirmDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd"
                                placeholder="预计进东兴时间" class="formWidth" style="width:277px"
                                :disabled="editFlag === 0"></el-date-picker>
              </template>
          ...
          <el-table-column label="确认人" prop="confirmUserName">
            <template slot-scope="{ row }">
              <template v-if="editFlag === 2">
                <el-input v-model="row.confirmUserName" class="edit-input"/>
              </template>
              <span v-else>{{ row.confirmUserName }}</span>
            </template>
          </el-table-column>
        </el-table>
        <el-form :model="driverForm" :inline="true" label-width="" class="mt30">
          <el-row>
            <el-col :span="24">
              <el-form-item label="24小时核酸证明" class="form_height">
            ...
            <el-col :span="16" v-if="driverForm['userReport'] && driverForm['userReport'].permitAudit===2 ">
              <el-form-item label="驳回原因" prop="rejectReason">
                <el-input disabled type="textarea" :autosize="{ minRows: 3, maxRows: 10}" placeholder="请输入驳回原因"
        ...
        <el-form :model="passForm" ref="passForm" :inline="true" label-width="" class="mt30" v-if="isShowRadio"
                 :rules="rules">
          <el-row>
            <el-col :span="24">
      ...
      <async-modal :dialogVisible="peerCntVisible" :title="modalTitle" @closeDialog="closeDialogUser" :width="'1100px'">
        <el-table v-loading="loading" :data="peerCntList" :header-cell-style="{'text-align':'center'}">
          <el-table-column label="序号" align="center" type="index" width="80"/>
          <el-table-column label="姓名" align="center" prop="name"/>
          <el-table-column label="联系电话" align="center" prop="phone"/>
          <el-table-column label="证件类型" align="center" prop="documentType">
            <template slot-scope="scope">
              {{ scope.row.documentType === 1 ? '身份证' : '护照' }}
            </template>
          </el-table-column>
          <el-table-column label="证件号码" align="center" prop="idNumber"/>
          <el-table-column label="创建时间" align="center" prop="upCreateTime"/>
        </el-table>
        <template slot="footerBar">
          <!--            <el-button type="primary" @click="closeDialogUser()">确定</el-button>-->
          <el-button @click="closeDialogUser()">取消</el-button>
        </template>
      </async-modal>
      <async-modal :dialogVisible="objectiveVisible" :title="modalTitle" @closeDialog="closeObjective"
                   :width="'1100px'">
        <el-table v-loading="loading" :data="objectiveList" :header-cell-style="{'text-align':'center'}">
          <el-table-column label="序号" align="center" type="index" width="80"/>
          <el-table-column label="目的地名称" align="center" prop="spName"/>
          <el-table-column label="是否到达" align="center" prop="confirm">
            <template slot-scope="scope">
              {{ scope.row.confirm ? '是' : '否' }}
            </template>
          </el-table-column>
          <el-table-column label="到达时间" align="center" prop="confirmDate"/>
          <el-table-column label="确认人" align="center" prop="confirmUserName"/>
        </el-table>
        <template slot="footerBar">
          <el-button type="primary" @click="closeObjective()">确定</el-button>
          <el-button @click="closeObjective()">取消</el-button>
        </template>
      </async-modal>
      <!-- 一键流转归属 -->
      <async-modal :dialogVisible="circulationVisible" :title="modalTitle" @closeDialog="closeCirculation"
                   :width="'400px'">
        <el-form ref="companyForm" :model="companyForm" :rules="companyRules" label-width="100px">
          <el-form-item label="行政部门" prop="toCompanyId">
            <el-select v-model="companyForm.toCompanyName" placeholder="请选择行政部门">
        ...
        <template slot="footerBar">
          <el-button type="primary" @click="companySubmitForm()">确定</el-button>
          <el-button @click="closeCirculation()">取消</el-button>
        </template>
      </async-modal>
    </div>
...
import {
  getTruckDriver, saveTruckDriver, truckDriverDetail, truckDriverDel, batchAuditl, saveGoodstype, cascadeQuery, getArea
} from "@/api/driver/driver";
import {getBusinessList} from '@/api/hs/businessManage.js'

const AsyncModal = () => import( './AsyncModal')
const asyncSelector = () => import( './asyncSelector')
...
export default {
  name: 'Apply',
  dicts: ['iso_type', 'is_enter', 'goods_type', 'driver_type', 'permit_audit', 'sys_user_sex', 'nation',
    'hoh_relationship', 'permit_state', 'sys_sos', 'apply_head_state', 'user_type', 'user_profession',
  ...
  data() {
    return {
      ...
      companyRules: {
        toCompanyId: [
          {required: true, message: '型政部门不能为空', trigger: 'blur'}
        ]
      },
      treeOptions: [],
      isShowSelect: false,
  ...
  methods: {
    ...
    handleCirculation() {
      if (this.selectedData.length <= 0) {
        this.$modal.msgError('请选择要流转的数据')
        return false
      }
      this.circulationVisible = true;
      this.modalTitle = '选择货车司机归属'
    },
    //关闭流转归属弹框
    closeCirculation() {
    ...
    companySubmitForm() {
      ...
      this.$modal.confirm('本次操作会转移（' + this.selectedData.length + '条）归属为【' + this.companyForm.toCompanyName + "】,请谨慎操作!").then(() => {
        ...
        saveGoodstype({
          urIds: this.selectedData,
          companyId: this.companyForm.toCompanyId
        }).then(res => {
          loading.close();
    ...
    handleExamine(row) {
      ...
      this.driverDetail(row.urId, row)
      this.editFlag = 0
    ...
    toInfoPage(row) {
      this.modalTitle = '同乘人'
      this.peerCntVisible = true
      truckDriverDetail({id: row.urId}).then(response => {
        if (response.code !== 200) {
          this.$alert(response.msg, '查询详情失败', {
            confirmButtonText: '确定',
            type: 'error'
          });
          return false;
        } else {
          this.peerCntList = response.data.userReport.userPeers
        }
      }).catch(() => {
      })
    },
    ...
    toInfodestCnt(row) {
      ...
      truckDriverDetail({id: row.urId}).then(response => {
        if (response.code !== 200) {
          ...
          this.objectiveList = response.data.driverReportDests
        }
      }).catch(() => {
      })
    },
    ...
    getList() {
      ...
      getTruckDriver(this.queryParams).then(response => {
        if (response.code !== 200) {
          return false;
        } else {
    ...
    saveForm() {
      ...
      if (that.editFlag === 0) {
        ...
        this.driverForm.driverReportDests = this.tableList
        saveTruckDriver(this.driverForm).then(response => {
          if (response.code !== 200) {
            return false;
          } else {
    ...
    handleExport() {
      this.$confirm('是否导出全部数据?', '提示', {
        ...
        this.download('/ur/v2/driver/list/export', {
          ...this.queryParams
        }, `货车司机报备列表_${new Date().getTime()}.xlsx`)
      }).catch(() => {
    ...
    //获取货车司机详情
    driverDetail(id, row) {
      truckDriverDetail({id: id}).then(response => {
        if (response.code !== 200) {
          return false;
        } else {
          ...
          this.$set(this.driverForm.userReport, 'userState', parseInt(row.userState))
          ...
          this.tableList = response.data.driverReportDests
        }
        this.loading = false
    ...
    addLine() {
      ...
      if (isEmpty(this.tableList)) {
        this.tableList = [];
        this.tableList.push(emptyData);
      } else {
        this.tableList.concat(this.tableList.push(emptyData));
      }
    ...
    queryhandleNodeClick(val) {
      this.queryParams.originId = val.id
      var origin = this.getHierarchyData(this.originTreeList, [val.id], val.label)
      //console.log(origin)
      this.queryParams.origin = origin.toString().replaceAll(',', '/')
    },
    queryhandleNodeClick1(val) {
      this.driverForm.originId = val.id
      var origin = this.getHierarchyData(this.originTreeList, [val.id], val.label)
      //console.log(origin)
      this.driverForm.origin = origin.toString().replaceAll(',', '/')
    },
    clearCompany() {
      this.driverForm.origin = null
    }
  },
...
.masterTable ::v-deep .el-link {
  color: #78B8F9
  //  border-bottom: 1px solid #551A8B
}

.masterTable ::v-deep .el-link.master-info .el-link--inner:hover {
  // border-bottom: 1px solid #78B8F9;
  color: #78B8F9
}

.masterTable ::v-deep .el-link.master-info:visited .el-link--inner:hover {
  color: #7a3ab5
}
