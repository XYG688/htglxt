<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="是否进入东兴" prop="enterState" style="margin-left:10px">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-delete"
           :disabled="multiple"
           v-hasPermi="['permit:entryPeople:remove']"
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
           :disabled="multiple"
          @click="handleAudit"
          v-hasPermi="['permit:entryPeople:audit']"
        >一键审核
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleReject"
          v-hasPermi="['permit:entryPeople:reject']"
        >一键驳回
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['permit:entryPeople:export']"
        >导出
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleSet"
          v-hasPermi="['permit:entryPeople:set']"
        >设置
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="driverList"  @selection-change="selectionChangeHandle" class="masterTable">
      ...
      <el-table-column label="姓名" align="center" prop="name"/>
       <!-- <el-table-column label="证件类型" align="center" prop="documentType">
          <template slot-scope="scope">
           {{parseDocumentType(scope.row.documentType)}}
         </template>
      </el-table-column>
      ...
      <el-table-column label="联系方式" align="center" prop="phone"/>
       ...
       <el-table-column label="入境口岸" align="center" prop="entryPort">
       </el-table-column>
       <el-table-column label="入境时间" align="center" prop="entryPortDate">
          <template slot-scope="scope">
           {{ parseTime(scope.row.entryPortDate, "{y}-{m}-{d}")}}
         </template>
       </el-table-column>
        <el-table-column label="预计进东兴时间" align="center" prop="enterDate">
          <template slot-scope="scope">
            {{ parseTime(scope.row.enterDate, "{y}-{m}-{d}")}}
          </template>
       </el-table-column>
       <el-table-column label="预计进东兴交通方式" align="center" prop="transportation">
          <template slot-scope="scope">
           <dict-tag :options="dict.type.user_transportation" :value="scope.row.transportation"/>
         </template>
       </el-table-column> -->
       <el-table-column label="航班(车次/车牌)" align="center" prop="flight">
       </el-table-column>
       <!-- <el-table-column label="目的地地址" align="center" prop="destAddrDetail">
       </el-table-column> -->
       <el-table-column label="入东兴目的" align="center" prop="enterPurpose">
          <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['permit:entryPeople:edit']"
          >修改
          </el-button>
          <el-button
            size="mini"
    ...
    />
    <div class="other">
      	<async-modal :dialogVisible="dialogVisible" :title="modalTitle" @closeDialog="closeDialog" :width="'1300px'" :isShowScroll="true">
           <el-form :model="driverForm" ref="queryForm" :inline="true" v-show="showSearch" label-width="260px">
            <el-row>
                <el-col :span="12" >
                  ...
                  <el-form-item label="证件类型" prop="permitAudit" >
                     <el-select v-model="driverForm.documentType" placeholder="请选择证件类型" clearable size="small"  style="width:320px" disabled>
                        <el-option
                  ...
                  <el-form-item label="入境口岸" prop="entryPort">
                    <el-input v-model="driverForm.entryPort" class="formWidth" :disabled="editFlag === 0"/>
                   </el-form-item>
                   <el-form-item label="入境前14日居住或途径的国家和地区" prop="passedArea" >
                      <el-input v-model="driverForm.passedArea" class="formWidth" :disabled="editFlag === 0"/>
                  </el-form-item>
                  <el-form-item label="预计进东兴时间" prop="enterDate" >
                    <el-date-picker  v-model="driverForm['userReport'] && driverForm['userReport'].enterDate" type="date" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd" placeholder="请输入预计进东兴时间" class="formWidth" style="width:320px" :disabled="editFlag === 0"> </el-date-picker>
                   </el-form-item>
                    <el-form-item label="航班(车次/车牌)" prop="flight">
                    <el-input v-model="driverForm.flight" class="formWidth" :disabled="editFlag === 0" />
                   </el-form-item>
                  <el-form-item label="入东兴后目的" prop="enterPurpose">
                    <el-select v-model="driverForm['enterPurpose']" placeholder="请选择入东兴后目的" clearable size="small" :disabled="editFlag === 0">
                      <el-option
                     ...
                     <!-- <async-selector v-model="driverForm.enterPurpose" :address="'/system/dict/data/type/user_profession'" placeholder="请选择入东兴后目的" :disabled="editFlag === 0"></async-selector> -->
                   </el-form-item>
                   <el-form-item label="是否进入东兴" prop="enterState" >
                    <el-select v-model="driverForm['userReport'] && driverForm['userReport'].enterState" placeholder="请选择是否进入东兴" class="formWidth" style="width:320px" :disabled="editFlag === 0">
                        <el-option
                          v-for="dict in dict.type.is_enter"
                          :key="dict.value"
                          :label="dict.label"
                 ...
                 <el-col :span="12" >
                   ...
                   <el-form-item label="证件号码" prop="idNumber">
                    ...
                    <el-form-item label="入境时间" prop="entryPortDate" >
                    <el-date-picker  v-model="driverForm['entryPortDate']" type="date" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd" placeholder="请输入入境时间" class="formWidth" style="width:320px" :disabled="editFlag === 0"> </el-date-picker>
                   </el-form-item>
                   <el-form-item label="解除隔离日期" prop="releaseDate" >
                    <el-date-picker  v-model="driverForm['releaseDate']" type="date" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd" placeholder="请输入解除隔离日期" class="formWidth" style="width:320px" :disabled="editFlag === 0"> </el-date-picker>
                   </el-form-item>
                    <el-form-item label="预计进东兴交通方式" prop="transportation">
                      <el-select v-model="driverForm['transportation']" placeholder="请选择进东兴交通方式" clearable size="small" :disabled="editFlag === 0">
                        <el-option
                    ...
                    <el-form-item label="第一入境点城市" prop="firstEntryCity">
                      <el-input v-model="driverForm.firstEntryCity" class="formWidth" :disabled="editFlag === 0"/>
                   </el-form-item>
                  ...
                  <el-form-item label="社区网格" prop="companyId">
                     <el-select v-model="driverForm.companyName" placeholder="请选择村社区" style="width:320px" :disabled="editFlag === 0">
                     <el-option :value="driverForm.companyId" :label="driverForm.companyName" style="width: 260px;height:200px;overflow: auto;background-color:#fff">
                  <el-tree
                    v-if="treeOptions.length > 0"
                    :data="treeOptions"
                    :props="defaultProps"
           ...
           <el-row>
                <el-col :span="24" >
                   <el-form-item label="是否需要隔离" prop="" >
                     <el-select v-model="driverForm['userReport'] && driverForm['userReport'].iso" placeholder="请选择" class="formWidth" style="width:320px" :disabled="editFlag === 0">
                        <el-option
                     ...
                     <el-select v-model="driverForm['userReport'] && driverForm['userReport'].isoType" placeholder="请选择隔离方式" class="displayIn" style="width:260px" :disabled="editFlag === 0" v-show="driverForm['userReport'] && driverForm['userReport'].iso === true">
                         <el-option
                       ...
                       <el-select v-model="driverForm['userReport'] && driverForm['userReport'].isoSPId" filterable allow-create default-first-option :filter-method="filterMethodThrottle" @blur="inputOperationValueSel" class="displayIn" style="margin-left:10px;width:325px" :disabled="editFlag === 0 " v-show="isShowSelect">
                    <el-option v-for="item in businessList" :key="item.isoSPId" :label="item.isoSPName" :value="item.isoSPId">
                    </el-option>
                  </el-select>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="10.8" >
                  <el-form-item label="网格证状态" prop="userState" >
                     <el-select :disabled="editFlag === 0" v-model="driverForm['userReport'] && driverForm['userReport'].userState" placeholder="状态" clearable size="small"  style="width:320px">
                        <el-option
                          v-for="dict in dict.type.permit_state"
                          :key="dict.value"
                          :label="dict.label"
                ...
                <el-col :span="12.2">
                    <el-date-picker  v-model="driverForm['userReport'] && driverForm['userReport'].isoStartDate"  placeholder="开始时间"  style="width:260px;margin-left:14px"   type="date" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd" :disabled="editFlag === 0"> </el-date-picker>
                    <el-date-picker  v-model="driverForm['userReport'] && driverForm['userReport'].isoEndDate"  placeholder="结束时间"  style="margin-left:11px;width:195px" @change="jsTime(driverForm['userReport'].isoStartDate,driverForm['userReport'].isoEndDate)" type="date" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd" :disabled="editFlag === 0"> </el-date-picker>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="48小时核酸证明" class="form_height">
              <userInfoImageUpload v-if="imageUploadData" v-model="driverForm['userReport'] && driverForm['userReport'].hsUrl" :limit='1' :isShowTip="false" :imageUploadData = 'imageUploadData' :disabled="editFlag === 0"/>
            </el-form-item>
            <el-form-item label="行程卡" class="form_height">
              <userInfoImageUpload v-if="imageUploadData" v-model="driverForm['userReport'] && driverForm['userReport'].routeUrl" :limit='1' :isShowTip="false" :imageUploadData = 'imageUploadData' :disabled="editFlag === 0"/>
            </el-form-item>
            <el-form-item label="健康码" class="form_height">
              <userInfoImageUpload v-if="imageUploadData" v-model="driverForm['userReport'] && driverForm['userReport'].healthUrl" :limit='1' :isShowTip="false" :imageUploadData = 'imageUploadData' :disabled="editFlag === 0"/>
            </el-form-item>
            <el-form-item label="其他" class="form_height">
              <userInfoImageUpload v-if="imageUploadData" v-model="driverForm['userReport'] && driverForm['userReport'].otherUrl" :limit='1' :isShowTip="false" :imageUploadData = 'imageUploadData' :disabled="editFlag === 0"/>
            </el-form-item>
          </el-col>
          <el-col :span="16" v-if="driverForm['userReport'] && driverForm['userReport'].permitAudit===2 ">
            <el-form-item label="驳回原因"  prop="rejectReason">
              <el-input type="textarea" :disabled="true" :autosize="{ minRows: 3, maxRows: 10}" placeholder="请输入驳回原因" v-model="driverForm['userReport'].rejectReason" style="width:700px">
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        </el-form>
        <el-form :model="passForm" ref="passForm" :inline="true" label-width="" class="mt30" :rules="rules" v-if="isShowRadio">
           <el-row>
             <el-col :span="24">
           ...
           <el-row>
             <el-col :span="24">
              <el-form-item label="驳回原因" v-if="passForm.pass === false " prop="rejectReason">
                <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 10}" placeholder="请输入驳回原因" v-model="passForm.rejectReason" style="width:700px">
                </el-input>
              </el-form-item>
            </el-col>
          </el-row>
...
} from "@/api/entryPeople/entryPeople";
...
export default {
  name: 'Apply',
  dicts: ['iso_type','is_enter','user_transportation','user_enter_purpose','goods_type','driver_type','permit_audit',
    'sys_user_sex', 'nation', 'hoh_relationship', 'permit_state', 'sys_sos', 'apply_head_state', 'user_type',
    'user_profession', 'residence_type', 'user_census', 'document_type','dismiss_content','valid_report'],
  data() {
    return {
      handleSetOpen:false,
      reportSetForm:{
...
// 表单重置
    ...
    handleExport() {
      this.$confirm('是否导出全部数据?', '提示', {
        ...
        this.download('/ur/v2/global/list/export', {
          ...this.queryParams
        }, `国外入境报备列表_${new Date().getTime()}.xlsx`)
      }).catch(() => {
      ...
      handler(newVal,oldVal){
        if(!newVal){
          ...
          this.day = null
           ...
           if(this.driverForm.userState === null){
